...
```
Create figures for mIHC data. Initial analysis was done by collaborator in OHSU
...
#=============================
# read in densities
dens = read.csv(file = paste0(rootDir,"for_paper/table_mIHC_cell_densities.csv"), row.names = 1)
...
# Stacked bar plot
# df_melt = melt(dens, id.vars = c("patientID", "timepoint"), measure.vars = immuneCells)
...
res = round(res,3)
write.csv(cbind(cluster = rownames(res),res), row.names = F, file = paste0(rootDir,"Results/mIHC/table_mIHC_wilcox_pvalues_densities.csv"))
...
# Ratios
### Precalculated ratios from OHSU

```{r ratios, echo=FALSE, fig.width= 4, fig.height= 3 }
#===============
# read in pre-calculated ratios (from OHSU)
r = read.csv(file = paste0(rootDir,"for_paper/table_mIHC_ratios.csv"), row.names = 1)
...
#=============================
# read in percentages
perc = read.csv(file = paste0(rootDir,"for_paper/table_mIHC_percentages.csv"))
...
res = round(res,3)
 write.csv(cbind(cluster = rownames(res),res), row.names = F, file = paste0(rootDir,"Results/mIHC/table_mIHC_wilcox_pvalues_percentage.csv"))
...
#===========
### Ratio of mature/immature DCs
#"DC-LAMP+ DCs"/"DC-LAMP- DCs"???
# ratio calculated from densities
df = cbind(ratio = dens[,14]/dens[,15], dens[,c("patientID", "timepoint", "RECIST") ] )
#print(i)
     # two time points
    df = df %>% filter(timepoint != "C2D1")  

    ggpaired(df, x="timepoint",y="ratio", id = "patientID", line.size = .7, line.color = "RECIST",palette =  recistCol,
     title = "(from densities)", xlab="Time point", ylab = "Ratio of mature/immature DCs") +
       theme(legend.position="right") +
  stat_compare_means()+
      scale_x_discrete(labels=timeLab)
...
# ratio calculated from percentages
df = cbind(ratio = as.numeric(perc[,56])/as.numeric(perc[,57]), perc[,c("patientID", "timepoint", "RECIST") ] )
#print(i)
     # two time points
    df = df %>% filter(timepoint != "C2D1")  

    ggpaired(df, x="timepoint",y="ratio", id = "patientID", line.size = .7, line.color = "RECIST",palette =  recistCol,
     title = "(from percentage)", xlab="Time point", ylab = "Ratio of mature/immature DCs") +
       theme(legend.position="right") +
  stat_compare_means()+
      scale_x_discrete(labels=timeLab)
